...
const _getDeltaIndicator = (delta: number) => {
...
const _getDeltaColor = (delta: number): string => {
    if (Math.abs(delta).toFixed(1) === '0.0') {
        return 'text-[#fcd706]'
    } else if (delta > 0) {
...
const _getDeltaBorderColor = (delta: number): string => {
    if (Math.abs(delta).toFixed(1) === '0.0') {
...
const _getDeltaBackgroundColor = (delta: number): string => {
    if (Math.abs(delta).toFixed(1) === '0.0') {
...
const DeltaIndicator = ({ data }: { data: any[] }) => {
    ...
    return true && <div className="flex flex-col gap-2 justify-center items-center font-equinox  z-10 fixed top-[12%] right-1/4 pointer-events-none">
        <p className={`flex flex-col justify-center items-center tracking-widest lowercase ${_getDeltaColor(delta)} text-xs`}>
            {metric + ' | ' + latestYear}
            <span className='text-white'>{'worldwide'}</span>
        </p>
        <div className={`flex flex-col gap-2 justify-center items-center w-[5.5rem] h-[5.5rem] border-dashed rounded-full border-2 ${_getDeltaColor(delta)} ${_getDeltaBorderColor(delta)} ${_getDeltaBackgroundColor(delta)} bg-opacity-30`}>
            {previousVal && <>{_getDeltaIndicator(delta)}</>}
            <p className="justify-center items-center text-center text-sm">{delta} <span className="">{'since ' + previousYear}</span></p>
        </div>
    </div>
}
